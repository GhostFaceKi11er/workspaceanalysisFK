...
int main() {
    ...
    std::string treefilePath = "/home/haitaoxu/workspaceanalysis/M1_full_load_5.bt";
    ...
    if (ProgramProcessMode == ProgramProcessMode_Process){
        ...
        if (IKflag) { // 如果计算正运动学遍历不完，则继续进行逆运动学
            ...
            angleBoundaries.first = "y";
            angleBoundaries.second(0) = -1.57;
            angleBoundaries.second(1) = 1.57;

            auto eeType = WorkSpaceAnalysis::EEndEffectorType::RIGHTHAND;
            // 右手初始姿态与基坐标系平行
            Eigen::Matrix3d initPose = Eigen::Matrix3d::Identity();
            double angleResolution = 0.1;

            Eigen::Vector6d posBoundaries = Eigen::Map<Eigen::Vector6d>(samplebound.data());
            analyzer.AnalyzeCartSpace(eeType, posBoundaries, resolution, initPose, angleBoundaries, angleResolution);
        }
    }
